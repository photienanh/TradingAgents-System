...
def _init_vnstock_auth() -> None:
    """Register vnstock user from environment variable at startup."""
    ...
    try:
        ...
        logger.warning("Không import được vnstock.register_user: %s", exc)
        return

    try:
        register_user(api_key)
        logger.info("Đăng nhập vnstock thành công qua VNSTOCK_API_KEY.")
    except Exception as exc:
        logger.warning("Đăng nhập vnstock thất bại: %s", exc)
...
def _fuse_decision_with_alphagpt(
    ...
    """
    Conditional fusion: AlphaGPT chỉ override TradingAgents khi signal đủ mạnh.

        Điều kiện để alpha can thiệp:
            1. ic_oos >= ic_threshold
            2. side=long  thì score > long_score_threshold   (mặc định > 0.5)
            3. side=short thì score < short_score_threshold (mặc định < -1.0)

    Nếu không đủ ngưỡng: giữ nguyên quyết định TradingAgents.

    Fusion logic (khi đủ ngưỡng):
      long  + TA=HOLD → BUY   | long  + TA=SELL → HOLD | long  + TA=BUY  → BUY
      short + TA=HOLD → SELL  | short + TA=BUY  → HOLD | short + TA=SELL → SELL
    """
    ...
    if ic_oos < ic_threshold:
        return decision, (
            f"AlphaGPT weak: ic_oos={ic_oos:.4f} < {ic_threshold} "
            f"→ TA decision kept ({decision})"
        )
    ...
    if side == "long":
        if score <= long_score_threshold:
            return decision, (
                f"AlphaGPT long weak: z={score:.3f} <= {long_score_threshold} "
                f"→ TA decision kept ({decision})"
            )
        if decision == "HOLD":
            return "BUY", f"AlphaGPT long (ic_oos={ic_oos:.4f}, z={score:.3f}) upgraded HOLD → BUY"
        if decision == "SELL":
            return "HOLD", f"Conflict: TA=SELL vs AlphaGPT long (ic_oos={ic_oos:.4f}) → HOLD"
        return "BUY", f"TA=BUY aligned with AlphaGPT long (ic_oos={ic_oos:.4f}, z={score:.3f})"

    if side == "short":
        if score >= short_score_threshold:
            return decision, (
                f"AlphaGPT short weak: z={score:.3f} >= {short_score_threshold} "
                f"→ TA decision kept ({decision})"
            )
        if decision == "HOLD":
            return "SELL", f"AlphaGPT short (ic_oos={ic_oos:.4f}, z={score:.3f}) downgraded HOLD → SELL"
        if decision == "BUY":
            return "HOLD", f"Conflict: TA=BUY vs AlphaGPT short (ic_oos={ic_oos:.4f}) → HOLD"
        return "SELL", f"TA=SELL aligned with AlphaGPT short (ic_oos={ic_oos:.4f}, z={score:.3f})"

    return decision, "No fusion applied"
...
async def run_trading_analysis(
    ...
    try:
        ...
        fused_decision, fusion_note = _fuse_decision_with_alphagpt(
            ta_decision_label, alpha_signal
        )
