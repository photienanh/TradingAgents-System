...
def get_top_alphas_by_ic_oos(limit: int = 5) -> List[Dict[str, Any]]:
    """Pick top alphas globally by ic_oos from data/alpha_library.json."""
    alphas = _load_alpha_library()
    valid: List[Dict[str, Any]] = []
    for alpha in alphas:
        expression = str(alpha.get("expression", "")).strip()
        ic_oos = _safe_float(alpha.get("ic_oos"))
        sharpe = _safe_float(alpha.get("sharpe_oos"))
        ret = _safe_float(alpha.get("return_oos"))
...
def compute_ticker_signal(
    ...
    alphas = top_alphas if top_alphas is not None else get_top_alphas_by_ic_oos(limit=5)
...
def _refresh_ticker_market_data(ticker: str, market_data_dir: Path = MARKET_DATA_DIR) -> Tuple[bool, int]:
    """Best-effort incremental update using vnstock. Returns (changed, new_rows)."""
    ...
    after = len(merged)
    return after > before, max(after - before, 0)
...
def refresh_market_data_daily(tickers: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    for ticker in tickers:
        try:
            is_changed, n_new = _refresh_ticker_market_data(ticker)
            if is_changed:
...
def build_daily_signals_snapshot(tickers: Optional[List[str]] = None) -> Dict[str, Dict[str, Any]]:
    ...
    for ticker in tickers:
        snapshot[ticker] = compute_ticker_signal(ticker, top_alphas=top_alphas)
    return snapshot


def save_signals_snapshot(snapshot: Dict[str, Dict[str, Any]]) -> Path:
    SIGNALS_DIR.mkdir(parents=True, exist_ok=True)
    path = SIGNALS_DIR / f"alpha_signals_{date.today().strftime('%Y%m%d')}.json"
...
def run_daily_update(tickers: Optional[List[str]] = None) -> Dict[str, Any]:
    """Main daily task: refresh market data then rebuild alpha signal snapshot."""
